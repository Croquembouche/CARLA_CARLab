...
This revision removes thirteen authored scenery vehicles: ten previously rejected by the local parking-exit planner, plus three remote trucks 30.7–44.1 m outside the road network. Thirty-five original vehicle placements remain; Control Center separately suppresses misplaced sidewalk two-wheelers. Lane geometry, traffic signals, parking bay annotations and buildings are unchanged by the cleanup script.
...
The script validates all thirteen object names, their positions, mesh asset identities and owning level before deletion. It backs up the original map beside the script, saves only the persistent Town10 level, reloads it, and verifies the resulting actor set. Reapplying to an already cleaned map is a no-op. The previous complete ten-removal revision is also accepted for this update. Other partial or mismatched maps fail validation without saving.

## Control Center scenario

`scenario.json` preserves the active user's 28 managed actors, six Lincoln ego sensors, weather and configured destinations. Restore this with the matching Control Center version; the actor IDs may change. Actor conversion and sensor configurations are Control Center functionality and are not baked into the native map.
