...
try:
 ...
 for row in manifest['removed']:
  ...
  targets.append(a)
 present_names={a.get_name() for a in targets}
 expected_names={row['source'] for row in manifest['removed']}
 previous_names=set(manifest.get('previous_removed',[]))
 assert present_names in (set(),expected_names,expected_names-previous_names), 'Partially modified map: inspect before applying'
