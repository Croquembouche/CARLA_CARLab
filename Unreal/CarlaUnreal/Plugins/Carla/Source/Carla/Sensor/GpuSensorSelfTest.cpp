...
void RunSensorGpuSelfTest(UWorld* World)
{
  ...
  FTimerHandle Timer;
  World->GetTimerManager().SetTimer(Timer, [World, Origin, WeakOwner, WeakTarget, WeakOccluder, Layers]
  {
    ...
    CarlaGpuSensors::Submit(*WeakOwner.Get(), MoveTemp(Rays),
        [Reference = MoveTemp(Reference), WeakOwner, WeakTarget, WeakOccluder, Layers, CpuMs, Submitted]
